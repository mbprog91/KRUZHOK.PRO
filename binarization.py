...
def binar_image(image):
    img_process = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY).astype('uint8')
    _, thresh1 = cv2.threshold(img_process, 120, 255, cv2.THRESH_BINARY)
    _, thresh2 = cv2.threshold(img_process, 120, 255, cv2.THRESH_BINARY_INV)
    _, thresh3 = cv2.threshold(img_process, 120, 255, cv2.THRESH_TRUNC)
    _, thresh4 = cv2.threshold(img_process, 120, 255, cv2.THRESH_TOZERO)
    _, thresh5 = cv2.threshold(img_process, 120, 255, cv2.THRESH_TOZERO_INV)
    ...
    _, thotsu = cv2.threshold(img_process, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    gauss_process = cv2.GaussianBlur(img_process, (5, 5), 0)
    _, thotsu2 = cv2.threshold(gauss_process, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
